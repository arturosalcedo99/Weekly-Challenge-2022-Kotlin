...
fun main() {
    val steps = arrayListOf(10, 5, -2)
    calculatePosition(steps)
}

private fun calculatePosition(steps: ArrayList<Int>) {
    val robot = Robot()
    steps.forEach {
        robot.makeStep(it)
    }
    print("Final position -> (x: ${robot.x}, y: ${robot.y})")
}
